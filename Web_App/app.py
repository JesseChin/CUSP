...
import time
import live_status
from live_status import GPS_Reader
...
def home_page():
    GPS = GPS_Reader()
    isConnected = GPS.get_connected()
    if isConnected == True:
        storage = live_status.storage()
        sats = GPS.get_sattelites()
        time = datetime.now()
...
def settings_page():
    ...
    return render_template(
        "settings.html", form_data=form_data, active_page="settings_page"
    )
...
def config_page():
    ...
    form_data = {}
    return render_template(
        "config.html", form=form, form_data=form_data, active_page="config_page"
    )
...
def calculate():
    ...
    if request.method == "POST":
        try:
            ...
            return render_template(
                "settings.html", show_alert=True, active_page="settings_page"
            )
        ...
        Number_of_Captures = (
            4046.86
            * Field_Area
            / (Footprint_Width * Footprint_Height * (1 - Forward_Overlap))
        )
    ...
    return render_template(
        ...
        disBetCap=Distance_Between_Capture,
        disBetTrack=Distance_Between_Track,
        tBetCap=Time_Between_Capture,
        flightTime=Flight_Time,
        numCap=Number_of_Captures,
        numImg=Number_of_Images,
        areaPerHour=Area_per_Hour,
        ssr=Storage_Space_Requirement,
        active_page="settings_page",
    )
